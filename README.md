...
| [skill-publisher](skills/skill-publisher/) | 从本地 `.agents` 选择 skill 同步发布到仓库 |
...
可替换 `--skill` 为以下任一值：
`bilibili-keywords-scraper` `douyin-scraper` `weibo-scraper` `xiaohongshu-scraper` `imap-smtp-email` `feishu-cli-manager` `video-download` `seedance-prompt` `skill-publisher`
