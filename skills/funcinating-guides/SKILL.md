...
3. **查竞品拆解表**(供给侧线索,2026-08-17 接入):飞书多维表格「竞品Blog-News亮眼内容拆解」
   base=`JYO8bk3doa9YF5sZ0mWcPMXwnCe` / table=`tblk9g87FdpmFx3X`。
   筛 `借鉴优先级` ∈ {P0 直接抄作业, P1 排期跟进},取每条的 `锁定关键词(推断)`。
   ```
   lark-cli api GET /open-apis/bitable/v1/apps/JYO8bk3doa9YF5sZ0mWcPMXwnCe/tables/tblk9g87FdpmFx3X/records \
     --params '{"page_size":200}' --as user
   ```
   **命中的词若不在台账,先入台账再动笔**(逻辑键=关键词,先拉全量比对防重复):
   三轴状态-状态=`手动录入`、**决策列留空**(空=待判定,周一 AI 预判会接手、周二默认同意定案)、
   身份-词族/分层与同类词对齐、备注=`竞品 {站点} {发布日} 已发:{URL}`。
   ```
   POST .../tables/tblu9AIxGSTVIgC8/records/batch_create   # 每批≤100,校验 code==0
   ```
   **为什么要这一步**:GSC 只能告诉你已经有曝光的词,永远告诉不了你还没进场的战场。
   竞品每发一篇文章就等于替我们做了一次关键词调研,不接这根管子,那份调研就烂在拆解表里。
   实测(2026-08-17):Utopia Toys 两周 27 篇锁定的词,**在台账里命中 0 个**。
   **现阶段这一步是人工执行的**;它被写成"读表→比对→batch_create"三步纯机械动作,
   没有 LLM 判断,将来要自动化直接搬进周一脚本即可,判定仍走已有的默认同意制。

4. **发布后回填**(2026-08-03 列名清理后的口径,勿用旧短名):
...
4. **竞品缺口型(情报驱动)**:拆解表里 P0/P1 的词,挑「对手已占坑、我们没有承接页」的做。
   注意只抄**打法**不抄**品类**——对手写高达/手办,我们写盲盒/毛绒,句式可移植、内容必须自己的。
   已验证可移植的句式:`where to buy authentic {品类}`、`how to avoid bootleg {品类}`、
   `{A} vs {B}`、`best {品类} brands`、`are {品类} worth buying`、`{品类} under ${价}`。
