  ...
  return new Promise((resolve, reject) => {
    imap.search(criteria, (err, results) => {
      ...
      fetch.on('message', (msg) => {
        ...
        msg.once('end', () => {
          if (message.parts.length > 0 || message.attributes) {
...
function parseHeaderOnly(headerStr, attrs) {
  const header = Imap.parseHeader(headerStr || '');
  const from = header.from?.[0] || 'Unknown';
  const to = header.to?.[0];
  const subject = header.subject?.[0] || '(no subject)';
  const dateValue = header.date?.[0] || attrs?.date;
...
async function checkEmails(mailbox = DEFAULT_MAILBOX, limit = 10, recentTime = null, unreadOnly = false) {
  ...
  try {
    ...
    for (const item of sortedMessages) {
      const headerPart = item.parts.find(p => /^HEADER\.FIELDS/i.test(p.which)) || item.parts[0];
      const parsed = parseHeaderOnly(headerPart?.body, item.attributes);
...
async function fetchEmail(uid, mailbox = DEFAULT_MAILBOX) {
  ...
  try {
    ...
    const bodyPart = item.parts.find(p => p.which === '') || item.parts[0];
    const parsed = await parseEmail(bodyPart?.body || '');
...
async function downloadAttachments(uid, mailbox = DEFAULT_MAILBOX, outputDir = '.', specificFilename = null) {
  ...
  try {
    ...
    const item = messages[0];
    const bodyPart = item.parts.find(p => p.which === '') || item.parts[0];
    const parsed = await parseEmail(bodyPart?.body || '', true);
